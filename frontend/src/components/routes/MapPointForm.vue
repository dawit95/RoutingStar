<template>
  <v-flex xs12>
    ...
    <div>
      <v-list outlined v-for="(pointItem, idx) in pointedItems" :key="idx">
        <v-icon drak large right style="cursor: pointer;">mdi-drag-horizontal-variant</v-icon>

        <v-icon right large style="cursor: pointer;" @click="removePoint(pointItem.marker, idx)"
        ...
        <v-list-item outlined ma-0 pa-0 @click="forcheck(pointItem)">
          <v-list-item-content>
            ...
            <v-textarea
              v-model="pointItem.content"
              @click="activePoint(pointItem)"
              @mouseout="stopPoint(pointItem)"
              label="장소에대한 짧은설명"
              rows="1"
...
export default {
  ...
  methods: {
    ...
    onFileSelected(pointItem) {
      ...
      const idx = this.pointedItems.findIndex(
        (e) => e.lat == pointItem.lat && e.lng == pointItem.lng
      );
